...
- Provide configuration files and setup instructions"""
    ...
    def generate_code(self, prompt: str, model: str, action: str, conversation_history: List[Dict] = None) -> Dict[str, Any]:
        """Generate code or ask clarifying questions using DeepSeek API"""
        ...
        deepseek_model_name = self.model_mapping[model]
        messages = self._build_messages(prompt, action, conversation_history)
        
        try:
            # Use the OpenAI client format that works with DeepSeek
            ...
            if "401" in error_msg:
                ...
                error_msg = f"Model not found. Please verify the model name: {deepseek_model_name}"
            elif "429" in error_msg:
                error_msg = "Rate limit exceeded. Please try again later."
            elif "500" in error_msg or "503" in error_msg:
                error_msg = "DeepSeek service temporarily unavailable. Please try again later."
    ...
    def check_model_availability(self) -> Dict[str, bool]:
        """Check availability of all DeepSeek models"""
        availability = {}
        for model_name, deepseek_model_name in self.model_mapping.items():
            try:
